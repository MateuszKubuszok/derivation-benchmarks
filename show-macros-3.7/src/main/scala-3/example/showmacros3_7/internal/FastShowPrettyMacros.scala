...
class FastShowPrettyMacros(val q: Quotes)
  ...
  object ShowExpr extends ShowExprModule {
    ...
    def summonFastShowPretty[A: Type]: Option[Expr[FastShowPretty[A]]] =
      scala.quoted.Expr.summonIgnoring[FastShowPretty[A]](
        Symbol.classSymbol("example.showmacros3_7.FastShowPretty").companionModule.methodMember("derived")*
      )
  ...
  def deriveFastShowPretty[A: Type]: Expr[FastShowPretty[A]] = ShowExpr.FastShowPretty.instance[A] {
    (value, sb, indent, nesting) =>
      deriveShowingExpression(using ShowingContext.create[A](value, sb, indent, nesting))
  }
}
